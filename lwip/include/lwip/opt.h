...
/*
 * This file is distributed with LWIP, see the lwip directory for legal comments.
 */
 ...
 * (requires the ARP_QUEUEING option) 8 bytes per pbuf = 64 bytes */
#define MEMP_NUM_ARP_QUEUE	8

/* MEMP_NUM_IGMP_GROUP: The number of multicast groups whose network interfaces
 * can be members et the same time (one per netif - allsystems group -, plus one
 * per netif membership). (requires the LWIP_IGMP option) 20 bytes per group = 80 bytes */
...
/* TCP_SNDQUEUELOWAT: TCP writable bufs (pbuf count). This must be greater
